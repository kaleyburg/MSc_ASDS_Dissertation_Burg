\documentclass[preprint,12pt,authoryear]{elsarticle} % Use elsarticle class (preprint option, no natbib options needed)
...
\textbf{H1}: Political party will play a significant role in determining the frequency of environmental mentions in campaign leaflets, wit Green Party leaflets having the highest frequency of environmental mentions.

\textbf{H2}: Constituencies with higher past vote share for the Consersative party in the previous election will have a lower frequency of environmental mentions in campaign leaflets, as they are in less direct competition with the Green Party

\textbf{H3}: Constituencies with higher student counts and a lower median age will have a higher frequency of environmental mentions in campaign leaflets, as young and educated constituents are more likely to be concerned with environmental issues \citep{klinebergDemographicPredictorsEnvironmental1998}.

\textbf{H4}: Constituencies with more rural populations will have a lower frequency of environmental mentions in campaign leaflets, as rural populations are less likely to be engage with environmental behaviour \citep{freudenburgRuralUrbanDifferencesEnvironmental1991}.
...
%add in justification and explanation of outcome data

%the outcome data is converted into a proportion of the environmental mentions over the total words 

\paragraph{Controls}



%change this to show that its the simplified version of occupations base on this https://www.gov.uk/government/publications/understanding-a-workforces-socio-economic-background-for-change/simplifying-how-employers-measure-socio-economic-background-an-accompanying-report-to-new-guidance link

%urban rural is now proportion
%professional occupations also as a proportion of total population in constituency


In order to measure the frequency of environmental mentions in campaign leaflets, it is first necessary to provide justification for the importance of using continuous outcome data. The frequency of environmental mentions in campaign leaflets will be measured by counting the number of words that are related to the environment or climate change in each leaflet. This will be done using the Laver \& Garry Quanteda dictionary \citep{laverEstimatingPolicyPositions2000}, which contains a list of words that are associated with various political topics, including the environment. The dictionary will be used to identify and count the number of environmental words in each leaflet, providing a continuous measure of environmental mentions. This allows for nuance in exactly how much each factor is influencing the campaign. A binary measure, which is currently what exists in the dataset created with manual human coding of leaflets, would provide only a measure of predicting if the campaigner mentions the environment \textit{at all}. Although this is useful, it misses important insights that might arise from why a constituency might, for example, mention the environment 1 time versus 10 times.

Furthermore, the creation of this novel dataset will allow for comparison between counts of environmental mentions, compared to counts of other political topics. This allows for a complete analysis of the proportion of text dedicated to each political topic.

In addition to weather data, various demographic factors were used as controls. Control data was taken from the 2011 UK Census \citep{2011CensusOffice}, as this is the nearest census data to all of the elections in the dataset (2010, 2015, 2017, and 2019) as the 2021 census was not released yet. These controls include median age, student counts, and counts of individuals in occupational statuses as outlined by the census forum. In order to analyse educational attainment, the total number of students, regardless of employment status, was summed to collect a total number of students within a constituency. As income data was not readily available numerically in the 2011 census, occupational rank was be used in its place. These overarching categories are: Higher managerial, administrative and professional occupations, Large employers and higher managerial and administrative occupations, Higher professional occupations, Lower managerial, administrative and professional occupations, Intermediate occupations, Small employers and own account workers, Lower supervisory and technical occupations, Semi-routine occupations, Routine occupations, Never worked, Long-term unemployed, and Full-time students. These are pooled across both males and females and are included as count variables in the dataset.
...
\begin{wrapfigure}{l}{0.58\textwidth} % 'l' for left alignment, 0.58 of the text width
	\centering
	\includegraphics[width=0.58\textwidth]{Figure_1.png}
...
\begin{figure}[htbp]  % Use [htbp] for figure placement
	...
	\begin{minipage}[t]{0.335\textwidth}
		\centering
		\includegraphics[width=\textwidth,height=0.68\textheight,keepaspectratio]{Figure_3.png}
		\caption{Conservative Party}
	\end{minipage}\hfill
	\begin{minipage}[t]{0.335\textwidth}
		\centering
		\includegraphics[width=\textwidth,height=0.68\textheight,keepaspectratio]{Figure_4.png}
		\caption{Green Party}
	\end{minipage}\hfill
	\begin{minipage}[t]{0.335\textwidth}
		\centering
		\includegraphics[width=\textwidth,height=0.68\textheight,keepaspectratio]{Figure_5.png}
	...
	\begin{minipage}[t]{0.335\textwidth}
		\centering
		\includegraphics[width=\textwidth,height=0.68\textheight,keepaspectratio]{Figure_6.png}
		\caption{Liberal Democrats}
	\end{minipage}\hfill
	\begin{minipage}[t]{0.335\textwidth}
		\centering
		\includegraphics[width=\textwidth,height=0.68\textheight,keepaspectratio]{Figure_7.png}
...
\begin{figure}[htbp]  % Use [H] to place the figure exactly here
	\centering
	\includegraphics[width=\textwidth, height=0.7\textheight, keepaspectratio]{Figure_8.png} % Use full text width, adjust height
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth, height=0.6\textheight, keepaspectratio]{Figure_9.png} % Use full text width, adjust height
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth, height=0.6\textheight, keepaspectratio]{Figure_10.png} % Use full text width, adjust height
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{Appendix_Figure_3.png}
	\caption{}
	\label{fig:Appendix_Figure_3}
\end{figure}
...
\subsection{OCR and Dictionary Methods}
